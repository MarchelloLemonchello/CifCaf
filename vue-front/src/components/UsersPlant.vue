...
const props = defineProps({
  ...
  addInfoAndOpen: Function,
  nowTime: Number
})
const token = JSON.parse(localStorage.getItem('plantToken'))

const percent = computed(()=>{
  const p = Math.trunc((props.nowTime-props.plant.lastWateringTime*1000)/((props.plant.nextWateringTime - props.plant.lastWateringTime)*10))
  // console.log(p);
  return p>100?100:p<=0?0:p
});
// console.log(percent.value);

async function delitePlant() {
  const res = await axios.delete(`${import.meta.env.VITE_BASE_API}UserPlant/${props.plant.id}` ,
  { headers: {"Authorization" : `Bearer ${token}`} })
  if (res.status==200){
    props.updateNowTime()
    props.plansListUpdate()
  }
}

async function waterPlant() {
  const res = await axios.patch(`${import.meta.env.VITE_BASE_API}UserPlant/${props.plant.id}` , {} ,
  { headers: {"Authorization" : `Bearer ${token}`} })
  console.log(res);
  if (res.status==200){
...
const nextWateringTime = computed(()=>new Date(props.plant.nextWateringTime*1000).toLocaleString("ru-RU"))

</script>

<template>
  <li class="flex users-plant" :class="{'needWater':percent==100}">
    <div class="flex">
    ...
    <div class="flex users-plant__btn-wrapper">
      ...
      <button class="btn-reset" @click="addInfoAndOpen(props.plant)">
...
.needWater button:first-child path {
  fill: #5898d3;
}
...
.users-plant__progress-wrapper p {
  font-size: 10px;
}
